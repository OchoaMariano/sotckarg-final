...
// Si no hay una sesion iniciada, redirigimos al usuario a la pagina de login
if(!isset($_SESSION['nombre_usuario'])) {
  header("Location: ../index.php");
  exit();
}

// Obtenemos el nombre de usuario, el nombre del usuario y el id del rol
...
// Si el usuario no es un comprador, redirigimos al usuario a la pagina de login
if($rol_id != 2) {
  header("Location: ../index.php");
  exit();
}
...
// Si se envia el formulario de operacion
if(isset($_POST['comprar'])) {
  
  $id_producto = $_POST['id_producto'];
  $id_usuario_vendedor = $_POST['id_usuario'];
  $id_usuario_comprador = $id_usuario;
  $id_estado = 3;

  // Insertar operacion en la base de datos
  $sql_c = "INSERT INTO Operacion (id_producto, id_usuario_vendedor, id_usuario_comprador, id_estado) 
  VALUES ('$id_producto', '$id_usuario_vendedor', '$id_usuario_comprador', '$id_estado')";
...
if(isset($_POST['reservar'])) {
  ...
  // Insertar operacion en la base de datos
  $sql_r = "INSERT INTO Operacion (id_producto, id_usuario_vendedor, id_usuario_comprador, id_estado) 
  VALUES ('$id_producto', '$id_usuario_vendedor', '$id_usuario_comprador', '$id_estado')";
  $result_r = mysqli_query($conexion, $sql_r);
  if($result_r) {
    echo "<script>alert('Operacion realizada con exito');</script>";
  } else {
    echo "<script>alert('Error al realizar la operacion');</script>";
...
>
  <head>
    ...
    <title>Dashboard - Vendedor | Stockarg</title>
                    ...
                    <div class="col">
                      <div class="card mb-4">
                        <div class="card-body">
                          ...
                          <h4 class="card-title text-nowrap mb-3">$<?= $producto['precio']; ?></h4>
                          ...
                          <button class="btn btn-sm btn-outline-primary" type="submit" name="reservar" class="card-link">Reservar</button>
                          <button data-bs-toggle="modal" data-bs-target="#smallModal" class="btn btn-sm btn-outline-success" type="submit" name="comprar" class="card-link">Comprar</button>
                        </div>
                      </div>
                    </div>
                    </form>
